...
export const ParticleCanvas = ({
  ...
  useEffect(() => {
    if (!running || intentField.length === 0 || !dimensions.width) return;
    
    const createParticlesInterval = setInterval(() => {
      if (particles.length >= maxParticles) return;
      
      setParticles(prevParticles => {
        // Skip creation if we're at max
        if (prevParticles.length >= maxParticles) return prevParticles;
  ...
  }, [running, intentField, dimensions, maxParticles, particleCreationRate, particles.length]);
  
  // Update intent field with fluctuations
  useEffect(() => {
    if (!running || intentField.length === 0) return;
  ...
  }, [running, intentField, intentFluctuationRate]);
  
  // Animation loop for particle movement and interactions
  useEffect(() => {
    if (!running || !canvasRef.current || intentField.length === 0) return;
  ...
  }, [running, intentField, dimensions, viewMode, learningRate, onStatsUpdate]);
