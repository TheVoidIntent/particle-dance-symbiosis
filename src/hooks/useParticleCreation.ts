...
export function useParticleCreation({
  ...
  const createNewParticles = useCallback((count: number = 1): Particle[] => {
    ...
    // Create a function that will be executed inside the setParticles
    const createParticles = (prevParticles: Particle[]): Particle[] => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (Math.random() < 0.1) {
          playSimulationEvent('particle_creation', { charge: newParticle.charge });
        }
    ...
    // Update the state with the new particles
